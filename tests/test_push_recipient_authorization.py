...
class _Context:
    def __init__(self, admin_ids: list[str], *, raises: bool = False) -> None:
        self.admin_ids = admin_ids
        self.raises = raises
        self.requests: list[str] = []

    def get_config(self, umo: str) -> dict[str, list[str]]:
...
class _Rules:
    def __init__(self) -> None:
        self._candidates = [
            types.SimpleNamespace(group_id="30003", platform_id="platform")
        ]
...
class PushRecipientAuthorizationTest(unittest.TestCase):
    def _command(
        ...
        *,
        ...
        context = _Context(admin_ids or [], raises=lookup_raises)
        config = _Config(
    ...
    def test_accepts_explicit_astrbot_administrator(self) -> None:
        ...
        self.assertEqual(context.requests, [_Event.unified_msg_origin])
        self.assertEqual(
            config.saved,
            [
