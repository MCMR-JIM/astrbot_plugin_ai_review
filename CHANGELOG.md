...
- **fix**：适配云黑插件 v3.0.0 的 `check_blacklist` 返回结构——命中标志为顶层 `in_blacklist`、数据在顶层 `data`；修正 `check_user` 云端查询解析避免漏判。新增 1 项云端口测试，共 167 项通过（9 项跳过）。

## v1.22.0

- **皮梦云黑库双向通信**：两插件通过 AstrBot 插件实例互调，实现双向协作。
  - AI 审核 → 皮梦云：通过处罚同步拉黑（已有方向，`api.add_to_blacklist`）。
  - 皮梦云 → AI 审核：审核前查询目标用户是否已在黑库（`service.is_user_blacklisted` 本地缓存优先，回退 `api.check_blacklist` 云端实时），命中则在 Prompt 中注明并**从严处理**（LLM 判定不违规时强制视为违规入队）。
- 新增配置 `enable_blacklist_check`（默认关，支持按群覆盖）。
- 版本 1.21 → 1.22；新增 9 项测试（黑库加重判定 5 项 + 适配器查询 4 项），共 166 项通过（9 项跳过）。
