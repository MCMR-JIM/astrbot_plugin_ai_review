...
> **版本规范**：版本号 = `大版本号.PR序号`。大版本号当前为 `1`；PR 序号为仓库已合并的 PR 总数。
> 例如当前 `1.20` 表示大版本 1、已合并 19 个 PR、本 PR 序号 20；每合并一个 PR，序号 +1。

AI 自动分析群成员聊天记录，为管理员生成审核建议（风险值、违规类型、证据、建议处罚），
并生成待审核任务；由 AstrBot 管理员（或在 `group_admin` 审批模式下，本群群主/群管）确认后执行处罚流水线。

> **重要**：AI 仅负责辅助审核，**不直接处罚用户**；所有处罚行为必须由 AstrBot 管理员（或在 `regex_approval_permission=group_admin` 模式下，本群群主/群管）通过 `/review pass` 人工确认后执行。
