...
class FakeBot:
    ...
    _ROLE_BY_ID = {"10001": "owner", "10002": "admin", "20001": "member"}
    ...
    async def call_action(self, action: str, **params):
        ...
        if action == "get_group_member_info":
            ...
            role = self._ROLE_BY_ID.get(user_id, "member")
...
class FakeContext:
    ...
    def get_config(self, umo):
        """模拟 AstrBot 会话配置（admins_id 全局字段）。"""
        return {"admins_id": []}
...
async def run() -> None:
    ...
    # ---------- 7. 群管鉴权（main 入口，group_admin 模式） ----------
    print("[7] 群管鉴权（group_admin 模式）")
    denied_messages = []
    async for r in plugin.cmd_review(
        FakeEvent("", sender="20001", message_str="/review pass deadbeef"),
        "pass", "deadbeef",
    ):
        denied_messages.append(r.message)
    check("普通成员 pass 被拒", any("权限不足" in m for m in denied_messages))
    ...
    # ---------- 9. 群主经入口 pass：处罚流水线 ----------
    print("[9] 群主 pass（入口鉴权 + 处罚流水线）")
    approved_messages = []
    async for r in plugin.cmd_review(
        FakeEvent("", sender="10001", message_str=f"/review pass {first_task.task_id}"),
        "pass", first_task.task_id,
    ):
        approved_messages.append(r.message)
    check("群主 pass 放行", any("已通过" in m for m in approved_messages))
    ...
    # 文本推送路径：阈值调回 3，单条候选 < 阈值 → 文本 + 群号前缀
    plugin.config._config["regex_forward_threshold"] = 3
